...
export function CustomerDetailScreen({ customerId }: { customerId: string }) {
  ...
  return (
    <>
      <PageHeader back={BACK} title={customer.name} />
      ...
      <div className={styles.contact}>
        {customer.phone && <Button size="sm" icon={<FiPhone />} onClick={() => (window.location.href = telLink(customer.phone))}>Call</Button>}
        {customer.phone && <Button size="sm" icon={<FiMessageCircle />} onClick={() => window.open(whatsappLink(customer.phone, reminder), "_blank", "noopener")}>WhatsApp</Button>}
